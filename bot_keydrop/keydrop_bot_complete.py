...
from input_utils import safe_float, safe_int, sanitize_str
from log_utils import setup_logger

logger = setup_logger("keydrop_gui")
...
class KeydropBotGUI:
    def __init__(self):
        ...
        try:
            ...
            logger.exception("Erro crítico na inicialização: %s", e)
    ...
    def setup_dark_theme(self):
        ...
        try:
            ...
            style.configure(
                "Large.TButton", font=("Arial", 14, "bold"), padding=(20, 15)
            )
            style.configure(
                "Emergency.TButton",
                font=("Arial", 14, "bold"),
            ...
            logger.info("Dark theme configurado com sucesso")

        except tk.TclError as e:
            logger.exception("Erro ao configurar dark theme: %s", e)
    ...
    def setup_icon(self):
        ...
        try:
            icon_paths = [
                ...
                os.path.expanduser(
                    "~/Desktop/Projeto do zero/bot_keydrop/bot-icone.ico"
                ),
            ]
    ...
    def create_loading_interface(self):
        ...
        try:
            ...
            tk.Label(
                loading_frame,
                text="🤖 Keydrop Bot Professional v3.0.0",
                font=("Arial", 18, "bold"),
    ...
    def setup_interface(self):
        ...
        try:
            ...
            tk.Label(
                header,
                text="Desenvolvido por William Medrado",
                font=("Arial", 12),
                bg=self.dark_colors["bg"],
                fg=self.dark_colors["fg"],
            ).pack()
            ...
            self.status_label = tk.Label(
                footer,
                text="📱 Modo Desktop Nativo • Pronto para uso",
                bg=self.dark_colors["bg"],
    ...
    def create_control_tab(self):
        ...
        tk.Label(
            info_frame,
            text="🤖 Automação Profissional para Sorteios Keydrop",
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["accent"],
        ...
        tk.Label(
            info_frame,
            text="🌐 Google Chrome Exclusivo • Múltiplos Perfis • Multi-Instância",
            font=("Arial", 12),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ...
        tk.Label(
            ...
            font=("Arial", 11),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["warning"],
        ).pack(anchor=tk.W, padx=10, pady=2)

        # Controle de Automação
        bot_frame = tk.LabelFrame(
            control_frame,
            text="🚀 Controle de Automação",
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["accent"],
            font=("Arial", 12, "bold"),
        )
        ...
        tk.Label(
            bot_frame,
            text="✨ Sistema de Multi-Bots com Perfis Independentes",
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["success"],
            font=("Arial", 12, "bold"),
        ...
        tk.Button(
            ...
            command=self.start_bot_direct,
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["success"],
            fg="white",
            relief="raised",
            bd=3,
        ).pack(side=tk.LEFT, padx=15, ipadx=20, ipady=10)

        tk.Button(
            buttons_frame,
            text="⏹️ PARAR AUTOMAÇÃO",
            command=self.stop_bot_direct,
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["warning"],
            fg="white",
            relief="raised",
            bd=3,
        ).pack(side=tk.LEFT, padx=15, ipadx=20, ipady=10)
        ...
        tk.Button(
            buttons_frame,
            text="🚨 EMERGÊNCIA",
            command=self.emergency_stop_direct,
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["error"],
            fg="white",
            relief="raised",
            bd=3,
        ).pack(side=tk.RIGHT, padx=15, ipadx=15, ipady=10)
    ...
    def create_config_tab(self):
        ...
        tk.Label(
            config_grid,
            text="🤖 Número de Guias/Bots (1-100):",
            font=("Arial", 12, "bold"),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ...
        tk.Entry(
            config_grid,
            textvariable=self.num_tabs_var,
            width=15,
            font=("Arial", 12),
            bg=self.dark_colors["entry_bg"],
            fg=self.dark_colors["entry_fg"],
            insertbackground=self.dark_colors["entry_fg"],
        ).grid(row=0, column=1, padx=15, pady=8)

        # Velocidade
        tk.Label(
            config_grid,
            text="⚡ Velocidade de Execução (segundos):",
            font=("Arial", 12, "bold"),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ...
        tk.Label(
            config_grid,
            text="💡 Recomendado: 7-10 segundos para máxima eficiência",
            font=("Arial", 11),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["success"],
        ...
        tk.Checkbutton(
            modes_inner,
            text="🕶️ Modo Headless (invisível)",
        ...
        tk.Label(
            modes_inner,
            text="• Headless: Bots funcionam em segundo plano (recomendado para muitos bots)",
            font=("Arial", 10),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ...
        tk.Label(
            modes_inner,
            text="• Mini: Janelas pequenas visíveis (bom para monitoramento)",
            font=("Arial", 10),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ).pack(anchor=tk.W, padx=20, pady=2)
        tk.Label(
            modes_inner,
            text="• Login: Abre páginas de login para autenticação manual",
            font=("Arial", 10),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ).pack(anchor=tk.W, padx=20, pady=2)
    ...
    def create_stats_tab(self):
        ...
        # Primeira linha
        self.total_bots_label = tk.Label(
            global_stats,
            text="🤖 Bots Ativos: 0",
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["bg"],
        ...
        self.total_raffles_label = tk.Label(
            global_stats,
            text="🎯 Total Sorteios: 0",
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["success"],
        )
        ...
        self.total_errors_label = tk.Label(
            global_stats,
            text="⚠️ Total Erros: 0",
            font=("Arial", 14, "bold"),
            bg=self.dark_colors["bg"],
        ...
        self.network_usage_label = tk.Label(
            global_stats,
            text="🌐 Uso Rede: 0 MB",
            font=("Arial", 12),
            bg=self.dark_colors["bg"],
        ...
        self.total_skins_label = tk.Label(
            global_stats,
    ...
    def create_logs_tab(self):
        ...
        # Header dos logs
        logs_header = tk.LabelFrame(
            logs_frame,
            text="📊 Sistema de Logs Avançado",
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["accent"],
            font=("Arial", 12, "bold"),
        )
        ...
        tk.Label(
            logs_header_inner,
            text="📜 Monitoramento em tempo real de todas as atividades do sistema",
            font=("Arial", 12),
            bg=self.dark_colors["bg"],
            fg=self.dark_colors["fg"],
        ).pack(anchor=tk.W)
    ...
    def update_global_stats(self):
        ...
        try:
            total_amateur = sum(
                bot.get("raffles_amateur", 0) for bot in self.bot_stats.values()
            )
            total_contender = sum(
                bot.get("raffles_contender", 0) for bot in self.bot_stats.values()
            )
            total_raffles = total_amateur + total_contender
            total_errors = sum(bot.get("errors", 0) for bot in self.bot_stats.values())
            total_network = sum(
                bot.get("network_usage", 0) for bot in self.bot_stats.values()
            )
            total_skins = sum(
                bot.get("skin_balance", 0.0) for bot in self.bot_stats.values()
            )
            ...
            if hasattr(self, "total_bots_label"):
                self.total_bots_label.config(
                    text=f"🤖 Bots Ativos: {self.total_bots_active}"
                )
            if hasattr(self, "total_raffles_label"):
                self.total_raffles_label.config(
                    text=f"🎯 Total Sorteios: {total_raffles}"
                )
            ...
            if hasattr(self, "total_skins_label"):
                self.total_skins_label.config(
                    text=f"💰 Saldo Total: ${total_skins:.2f}"
                )
    ...
    def start_bot_direct(self):
        ...
        try:
            ...
            if num_tabs is None:
                ...
                return
            headless_mode = (
                self.headless_var.get() if hasattr(self, "headless_var") else False
            )
            mini_mode = (
                self.mini_window_var.get()
                if hasattr(self, "mini_window_var")
                else False
            )
            login_tabs = (
                self.login_tabs_var.get() if hasattr(self, "login_tabs_var") else False
            )

            self.log_message(
                f"📋 Configurações: {num_tabs} guias, Headless: {headless_mode}, Mini: {mini_mode}",
                "INFO",
            )
    ...
    def run_edge_automation(
        self, edge_path, num_tabs, headless_mode, mini_mode, login_tabs
    ):
        """Executar automação"""
        try:
            self.edge_processes = []
                ...
                try:
                    ...
                    if headless_mode:
                        ...
                        args.extend(
                            [
                                "--window-size=300,400",
                                f"--window-position={100+(i*50)},{100+(i*50)}",
                            ]
                        )
                    else:
                        args.extend(
                            [
                                "--window-size=800,600",
                                f"--window-position={100+(i*100)},{100+(i*50)}",
                            ]
                        )
                    ...
                    self.log_message(
                        f"✅ Bot #{i+1} iniciado! PID: {process.pid}", "SUCCESS"
                    )
                    time.sleep(1)
            ...
            self.log_message(
                f"🎉 {len(self.edge_processes)} bots iniciados!", "SUCCESS"
            )
    ...
    def emergency_stop_direct(self):
        ...
        try:
            result = messagebox.askyesno(
                "Emergência", "Fechar TODOS os processos Chrome?"
            )
            if result:
                ...
                for proc in psutil.process_iter(["pid", "name"]):
                    try:
                        ...
                        pass
                self.log_message(
                    f"🛑 {killed} processos Chrome finalizados!", "WARNING"
                )
        except Exception as e:
            self.log_message(f"❌ Erro: {e}", "ERROR")
