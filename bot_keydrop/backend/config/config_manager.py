...
class BotConfig(BaseModel):
    ...
    num_tabs: int = Field(default=5, ge=1, le=100, description="Número de guias do Chrome")
    execution_speed: float = Field(default=1.0, ge=0.1, le=10.0, description="Velocidade de execução (multiplicador)")
    retry_attempts: int = Field(default=5, ge=1, le=20, description="Número de tentativas em caso de falha")
    ...
    steam_login_url: str = Field(default="https://steamcommunity.com/login/home/?goto=", description="URL de login Steam")
    ...
    max_memory_usage_mb: int = Field(default=2048, ge=512, le=8192, description="Uso máximo de memória (MB)")
    enable_cache_clearing: bool = Field(default=True, description="Habilitar limpeza automática de cache")
