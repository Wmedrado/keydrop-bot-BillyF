...
class KeydropBot:
    ...
    def setup_driver(self):
        ...
        if not SELENIUM_AVAILABLE:
            raise Exception("Selenium não está disponível ou não foi importado corretamente")
        ...
        if EdgeOptions and EdgeService and EdgeChromiumDriverManager and webdriver:
            try:
                browser_attempts.append(('edge', 'Microsoft Edge', EdgeOptions, EdgeService, EdgeChromiumDriverManager, webdriver.Edge))
        ...
        if FirefoxOptions and FirefoxService and GeckoDriverManager and webdriver:
            try:
                browser_attempts.append(('firefox', 'Mozilla Firefox', FirefoxOptions, FirefoxService, GeckoDriverManager, webdriver.Firefox))
            except AttributeError:
                pass
        
        if not browser_attempts:
            raise Exception("Nenhum navegador disponível! Instale selenium e webdriver-manager")
        
        last_errors = []
        
        for browser_type, browser_name, OptionsClass, ServiceClass, DriverManager, WebDriverClass in browser_attempts:
            try:
                ...
                if not all([OptionsClass, ServiceClass, DriverManager, WebDriverClass]):
                    print(f"[Bot {self.bot_id}] ⚠️ {browser_name} não disponível (imports faltando)")
                    continue
                ...
                try:
                    ...
                    try:
                        print(f"[Bot {self.bot_id}] Tentando {browser_name} do PATH do sistema...")
                        self.driver = WebDriverClass(options=options)
                        driver_created = True
                        print(f"[Bot {self.bot_id}] ✅ {browser_name} configurado do PATH")
                    except Exception as e2:
                        print(f"[Bot {self.bot_id}] ❌ PATH falhou: {e2}")
                        last_errors.append(f"{browser_name}: [webdriver-manager: {e1}] [PATH: {e2}]")
                        continue
                
                if driver_created:
                    self.browser_used = browser_name
                    print(f"[Bot {self.bot_id}] 🎉 SUCESSO: {browser_name} está funcionando!")
                    ...
                    try:
                        ...
                        print(f"[Bot {self.bot_id}] ❌ Teste de conectividade falhou: {e}")
                        self.driver.quit()
                        continue
    ...
    def _configure_browser_options(self, options, browser_type):
        ...
        window_size = "400,300" if self.config.get('mini_window_mode', False) else "1024,768"
            ...
            while self.running:
                try:
                    ...
                    if self.participate_in_giveaways("AMATEUR"):
                        ...
                        self.stats['ultima_atividade'] = f'Aguardando {wait_time}s (AMATEUR)'
                ...
                try:
                    ...
                    for button in buttons:
                        try:
                            ...
                            print(f"[Bot {self.bot_id}] Erro ao clicar em botão AMATEUR: {e}")
                            continue
                ...
                try:
                    ...
                    for button in buttons:
                        try:
                            ...
                            print(f"[Bot {self.bot_id}] Erro ao clicar em botão CONTENDER: {e}")
                            continue
    ...
    def handle_error(self, error):
        ...
        try:
            ...
            if retry_count >= max_retries:
                print(f"[Bot {self.bot_id}] Reiniciando driver após {max_retries} tentativas")
                self.restart_driver()
...
class KeydropBotGUI:
    ...
    def setup_interface(self):
        ...
        title_label = tk.Label(header, text="🤖 Keydrop Bot Professional v4.0.0", 
                              font=('Arial', 18, 'bold'), fg='white', bg='#2c3e50')
        title_label.pack(pady=10)
        
        subtitle_label = tk.Label(header, text="Desenvolvido por William Medrado (wmedrado)", 
    ...
    def create_control_tab(self):
        ...
        main_control = ttk.LabelFrame(control_frame, text="Controle Principal", padding=15)
        ...
        tk.Button(button_frame, text="🚀 Iniciar Bots", command=self.start_bots,
                 font=('Arial', 12, 'bold'), bg='#27ae60', fg='white', 
                 width=15, height=2).pack(side='left', padx=5)
        
        tk.Button(button_frame, text="⏹️ Parar Bots", command=self.stop_bots,
                 font=('Arial', 12, 'bold'), bg='#e74c3c', fg='white', 
                 width=15, height=2).pack(side='left', padx=5)
        
        tk.Button(button_frame, text="🚨 EMERGÊNCIA", command=self.emergency_stop,
        ...
        self.status_tree = ttk.Treeview(status_frame, columns=('Status', 'Participações', 'Erros', 'Última Atividade'), show='tree headings')
    ...
    def create_config_tab(self):
        ...
        basic_frame = ttk.LabelFrame(config_frame, text="Configurações Básicas", padding=10)
        basic_frame.pack(fill='x', padx=10, pady=5)
        
        # Grid para configurações
        tk.Label(basic_frame, text="Número de Guias (1-100):").grid(row=0, column=0, sticky='w', pady=5)
        self.num_tabs_var = tk.StringVar(value=str(self.config['num_tabs']))
        tk.Entry(basic_frame, textvariable=self.num_tabs_var, width=10).grid(row=0, column=1, padx=5, pady=5)
        
        tk.Label(basic_frame, text="Velocidade de Execução (seg):").grid(row=1, column=0, sticky='w', pady=5)
        self.speed_var = tk.StringVar(value=str(self.config['execution_speed']))
        tk.Entry(basic_frame, textvariable=self.speed_var, width=10).grid(row=1, column=1, padx=5, pady=5)
        
        tk.Label(basic_frame, text="Tentativas de Retry:").grid(row=2, column=0, sticky='w', pady=5)
        self.retry_var = tk.StringVar(value=str(self.config['retry_attempts']))
        tk.Entry(basic_frame, textvariable=self.retry_var, width=10).grid(row=2, column=1, padx=5, pady=5)
        
        # Opções avançadas
        advanced_frame = ttk.LabelFrame(config_frame, text="Opções Avançadas", padding=10)
        advanced_frame.pack(fill='x', padx=10, pady=5)
        
        self.headless_var = tk.BooleanVar(value=self.config['headless_mode'])
        tk.Checkbutton(advanced_frame, text="Modo Headless (invisível)", 
                      variable=self.headless_var).pack(anchor='w', pady=2)
        
        self.mini_window_var = tk.BooleanVar(value=self.config['mini_window_mode'])
        tk.Checkbutton(advanced_frame, text="Modo Mini (400x300px)", 
                      variable=self.mini_window_var).pack(anchor='w', pady=2)
        
        self.login_tabs_var = tk.BooleanVar(value=self.config['enable_login_tabs'])
        tk.Checkbutton(advanced_frame, text="Abas de Login (Keydrop/Steam)", 
                      variable=self.login_tabs_var).pack(anchor='w', pady=2)
        
        # Discord
        discord_frame = ttk.LabelFrame(config_frame, text="Integração Discord", padding=10)
        discord_frame.pack(fill='x', padx=10, pady=5)
        
        tk.Label(discord_frame, text="Webhook URL:").pack(anchor='w')
        self.discord_webhook_var = tk.StringVar(value=self.config['discord_webhook_url'])
    ...
    def create_stats_tab(self):
        ...
        # Sistema
        system_frame = ttk.LabelFrame(stats_frame, text="Performance do Sistema", padding=10)
        ...
        bot_stats_frame = ttk.LabelFrame(stats_frame, text="Estatísticas dos Bots", padding=10)
        bot_stats_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        self.stats_text = scrolledtext.ScrolledText(bot_stats_frame, height=20, state='disabled')
    ...
    def create_logs_tab(self):
        ...
        tk.Button(controls_frame, text="🗑️ Limpar Logs", 
                 command=self.clear_logs, font=('Arial', 10), 
                 bg='#e74c3c', fg='white').pack(side='left', padx=5)
        
        tk.Button(controls_frame, text="💾 Salvar Logs", 
                 command=self.save_logs, font=('Arial', 10), 
    ...
    def emergency_stop(self):
        ...
        result = messagebox.askyesno("Parada de Emergência", 
    ...
    def update_system_stats(self):
        ...
        try:
            ...
            self.cpu_label.config(text=f"🖥️ CPU: {cpu_percent:.1f}% ({psutil.cpu_count()} núcleos)")
            
            # RAM
            memory = psutil.virtual_memory()
            self.ram_label.config(text=f"💾 RAM: {memory.used // (1024*1024)} MB / {memory.total // (1024*1024)} MB ({memory.percent:.1f}%)")
            
            # Disco
            disk = psutil.disk_usage('C:\\')
            self.disk_label.config(text=f"💿 Disco C: {disk.used // (1024*1024*1024)} GB / {disk.total // (1024*1024*1024)} GB ({disk.percent:.1f}%)")
...
"""
            ...
            for bot_id, bot in self.bot_manager.bots.items():
                ...
                # Adicionar à árvore
                self.status_tree.insert('', 'end', text=f'Bot {bot_id}',
                                      values=(status, 
...
def main():
    ...
    try:
        ...
        try:
            ...
            tk.Label(root, text="❌ Erro ao Iniciar Aplicação", 
                    font=('Arial', 16, 'bold'), fg='red').pack(pady=20)
            
            tk.Label(root, text="A aplicação encontrou um erro:", 
